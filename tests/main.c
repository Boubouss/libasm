...
void  test_strcpy(const char* str) {
  ...
  printf("right sized buffer (<= %zu)\n", strlen(str));
  char* buf = malloc(strlen(str) + 1);
  if (!buf) {
    printf("malloc failed on strcpy test\n");
    return;
  }
  strcpy(buf, str);
  printf("test strcpy :\nsrc : '%s'\ndest : '%s'\n", str, buf);
...
void test_strdup(const char* str) {
  ...
  char* dup = strdup(str);
  if (!dup) {
    printf("strdup failed on strcdup test\n");
    return;
  }
  ...
  dup = ft_strdup(str);
  if (!dup) {
    printf("ft_strdup failed on strcdup test\n");
    return;
  }
  printf("ft_strdup : \nsrc : '%s' => %zu\ndup : '%s' => %zu\n\n", str, strlen(str), dup, strlen(dup));
  free(dup);
...
void    test_read(const char* str) {
  ...
  if (fd < 0) {
    printf("creation of file failed on test_read\n\n");
    return;
  }
  ...
  if (!buf) {
    close(fd);
    printf("malloc failed on test_read\n\n");
    return;
  }
  ...
  printf("read : %zd\n", read(-1, buf, strlen(str)));
  printf("errno => %d\n", errno);
  
  printf("ft_read : %zd\n", ft_read(-1, buf, strlen(str)));
  printf("errno => %d\n\n", errno);
